...
export const singupSchemachema = z.object({
    ...
    fullName: z.string().min(5, 'Nome muito curto').max(50, 'Nome muito grande').transform(v => v.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')),
